...
默认设备已经写死 TB300FU（serial `HGR496PP`），直接调用即可：
...
# 点右下角（Debug 齿轮应在的位置，横屏逻辑坐标约 1280x800）
$SCRIPT tap 1200 760
```
...
4. **设备坐标**：TB300FU 物理 800x1280（竖屏），DeskMario 强制横屏后 Flutter 内部旋转 90°，所以 Debug 右下角入口对应横屏逻辑坐标约 `(1200, 760)`，不是竖屏坐标 `(700, 100)`。
