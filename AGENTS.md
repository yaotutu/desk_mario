...
| [docs/notifications.md](docs/notifications.md)         | 调整 4 个严重度消息行为 / 修通知 bug       |
...
**核心规则：所有开发最终都要测试，测试时必须用真机的运行截图进行查看。** 任何 UI / 动画 / 交互改动，都要在 TB300FU 真机上跑一遍并截图确认，不能只靠 `flutter analyze` + `flutter test` 就算完事。

工具入口统一走 `.agents/skills/android-dev/scripts/adb.sh`（封装 adb，10 个高频子命令）：
...
$SCRIPT tap <x> <y>              # 点击（横屏后逻辑坐标约 1280×800）
$SCRIPT swipe x1 y1 x2 y2 [ms]   # 滑动
$SCRIPT logcat                   # Flutter 日志（自动清 buffer）
$SCRIPT info                     # 设备 + 前台应用概览
```

**设备信息**：TB300FU，serial `HGR496PP`，Android 13，物理 800×1280（竖屏），density override 213。DeskMario 强制横屏后 Flutter 内部旋转 90°，逻辑坐标约 1280×800。
...
└── features/                       # 业务功能模块
    ├── parallax/widgets/           # L0 视差背景 + 地面 + 滚动世界顶层
    ├── weather/widgets/            # L1 天气效果（占位）
    ├── character/widgets/          # L2 Mario 主角
    ├── hud/                        # L3 时间 HUD
    ├── notifications/              # L4 4 个严重度（S1~S4）消息系统
...
- **Step 5**：接入真实通知源
- **Step 6**：✅ Severity 4 强告警的 `BackdropFilter` 灰度模糊接到 `LayeredScaffold`（已完成，详见 [docs/roadmap.md](docs/roadmap.md) §Step 6）
- **Step 7**：L1 WeatherLayer 接入 weatherProvider（雨/雪/雾/闪电）


## 重要规则，由用户手动填写，优先级最高，并且禁止修改
- 所有的素材禁止绘制，必须是真实的。
